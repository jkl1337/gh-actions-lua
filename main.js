...
const exists = (filename, mode) =>
  fsp.access(filename, mode).then(
    () => true,
    () => false,
  );
...
async function msvc_link(luaExtractPath, linkCmd, outFile, objs) {
  ...
  if (await exists(manifest)) {
    await exec.exec(
      "mt /nologo",
      ["-manifest", manifest, "-outputresource:" + outFile],
      {
        cwd: luaExtractPath,
      },
    );
...
async function install_plain_lua_windows(
  ...
  await fsp.readdir(src).then(async (files) => {
    for (let file of files) {
      if (file.endsWith(".c")) {
        ...
        let args = mode === "lib" ? ["-DLUA_BUILD_AS_DLL", srcName] : [srcName];
  ...
  const luaHpp = (await exists(pathJoin(src, "lua.hpp")))
    ? "lua.hpp"
    : "../etc/lua.hpp";
...
async function install_plain_lua(luaInstallPath, luaVersion) {
  ...
  const luaSourceTar = await tc.downloadTool(
    `https://lua.org/ftp/lua-${luaVersion}.tar.gz`,
  );
  await io.mkdirP(luaExtractPath);
  await tc.extractTar(
    luaSourceTar,
    path.join(process.env["RUNNER_TEMP"], BUILD_PREFIX),
  );

  if (isWindows()) {
    return await install_plain_lua_windows(
      luaExtractPath,
      luaInstallPath,
      luaVersion,
    );
  ...
  await exec.exec(
    `make -j INSTALL_TOP="${luaInstallPath}" install`,
    undefined,
    {
      cwd: luaExtractPath,
    },
...
const makeCacheKey = (luaVersion, compileFlags) =>
  `lua:${luaVersion}:${process.platform}:${process.arch}:${compileFlags}`;
